...
namespace mapreduce {
    ...
    public:
        ...
        //
        //This is the underlying type of the data in the vector:
        //
        //template<class T, class Allocator = std::allocator<T>> class vector;
        //Member type   Definition
        //value_type  T
        using map_result_t = typename decltype(std::declval<Map>()("", ""))::value_type; // vector -> value_type
        //
        //std::pair has 2 member types:
        //template<class T1, class T2> struct pair;
        //Member type   Definition
        //first_type  T1
        //second_type T2
        //
        using map_value_t = typename decltype(std::declval<Map>()("", ""))::value_type::second_type;
        // vector<pair> -> second_type is value
        using reduce_result_t = decltype(std::declval<Reduce>()("", {}));
    ...
    private:
        ...
        void run_single_reducer(int start_i, int end_i, int thread_idx) {
            Reduce reducer(path_to_save_reduce_files + "reduce_" + std::to_string(thread_idx) + ".txt");
