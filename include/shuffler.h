...
template<typename K, typename V>
void shuffler<K, V>::run(std::vector<std::vector<std::pair<K, V>>>& after_map,
                         std::vector<std::vector<std::pair<K, V>>>& for_reduce)
{
    ...
    if (mapped_count % for_reduce.size() != 0) per_reducer_count += 1;
    // Traces current non-copied positions of each yet non-traversed container:
    ...
    // Try to extrapolate merge step of the merge sort algorithm to the case of multiple containers instead of 2
    while(!containers.empty())
    {
        // Find the first container with current copying position having key less then of all others containers' positions:
        ...
        {
            // Skip if it is not the case:
            if (container_it->m_pos->first != min_key) continue;
            ...
            // Advance current copying position for amount of copied elements:
